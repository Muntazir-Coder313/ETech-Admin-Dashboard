...
// Helper: fetch insights from API
const fetchInsightsFromAPI = async (filters = {}) => {
  ...
  // Filter by type (client‑side for demo – in real API you'd do it server‑side)
...
const InsightSkeleton = () => (
  <div className="bg-white/70 dark:bg-gray-900/70 backdrop-blur-sm rounded-2xl p-5 border animate-pulse">
    <div className="flex justify-between items-start">
      <div className="h-6 bg-gray-300 dark:bg-gray-700 rounded w-3/4"></div>
      <div className="flex gap-2"><div className="h-5 w-16 bg-gray-300 rounded-full"></div><div className="h-5 w-16 bg-gray-300 rounded-full"></div></div>
    </div>
...
const Insights = () => {
  ...
  const [sortBy, setSortBy] = useState('impact'); // 'impact', 'date', 'title'
  ...
  const handleApply = (id) => {
    ...
    // Optional: call API to log this application
    // fetch(`${API_BASE_URL}/insights/${id}/apply`, { method: 'POST' });
  };
  ...
  return (
    <div className="space-y-6 animate-fade-in">
      ...
      <div className="flex flex-wrap justify-between items-center gap-4">
        <div>
          <h1 className="text-4xl font-black bg-gradient-to-r from-gray-900 to-gray-600 dark:from-white dark:to-gray-400 bg-clip-text text-transparent">AI Insights</h1>
          <p className="text-gray-500 mt-1">Real‑time intelligence & recommendations</p>
        </div>
        <button 
          onClick={handleRefresh} 
          disabled={refreshing}
          className="bg-blue-600 hover:bg-blue-700 text-white px-4 py-2 rounded-xl shadow flex items-center gap-2 transition disabled:opacity-50"
        >
      ...
      <div className="flex flex-wrap gap-3 items-center justify-between">
        <div className="flex flex-wrap gap-2">
          <div className="bg-green-100 dark:bg-green-900/30 text-green-700 px-3 py-1 rounded-full text-sm flex items-center gap-1"><CheckCircle size={14}/> {positiveCount} Positive</div>
          <div className="bg-amber-100 dark:bg-amber-900/30 text-amber-700 px-3 py-1 rounded-full text-sm flex items-center gap-1"><AlertCircle size={14}/> {warningCount} Warnings</div>
          <div className={`bg-blue-100 dark:bg-blue-900/30 px-3 py-1 rounded-full text-sm flex items-center gap-1 ${totalImpact >= 0 ? 'text-green-700' : 'text-red-700'}`}>
            <DollarSign size={14}/> Total impact: {formatCurrency(totalImpact)}
          </div>
        </div>
        <div className="flex gap-2">
          <div className="relative">
            ...
            <input 
              ...
              className="pl-8 pr-3 py-1.5 border rounded-xl text-sm bg-white/70 dark:bg-gray-900/70 w-56" 
            />
          </div>
          <select 
            value={sortBy} 
            onChange={e => setSortBy(e.target.value)} 
            className="border rounded-xl px-2 py-1.5 text-sm bg-white/70 dark:bg-gray-900/70"
          >
          ...
          <button 
            ...
            className="border rounded-xl px-2 py-1.5 bg-white/70 dark:bg-gray-900/70"
          >
            <ArrowUpDown size={16} />
          </button>
        </div>
      ...
      <div className="flex gap-2 flex-wrap">
        {['all', 'positive', 'warning', 'neutral'].map(type => (
          <button 
            ...
            className={`px-4 py-1.5 rounded-full text-sm font-medium transition capitalize ${filterType === type ? 'bg-blue-600 text-white shadow-md' : 'bg-gray-200 dark:bg-gray-800 text-gray-700 dark:text-gray-300'}`}
          >
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
          {sortedInsights.map(insight => {
            ...
            return (
              <div 
                key={insight.id} 
                className={`relative overflow-hidden rounded-2xl p-5 shadow-lg hover:shadow-xl transition-all duration-300 bg-white/80 dark:bg-gray-900/80 backdrop-blur-sm ${styles.border} hover:scale-[1.02] group`}
              >
                <div className={`absolute top-0 right-0 w-32 h-32 bg-gradient-to-br ${styles.gradient} opacity-10 rounded-full blur-2xl group-hover:opacity-20 transition`} />
                <div className="flex justify-between items-start">
                  <div className="flex items-center gap-2">
                    <Icon className={`w-5 h-5 ${styles.iconColor}`} />
                    <h3 className="font-bold text-gray-800 dark:text-white text-lg">{insight.title}</h3>
                  </div>
                  <div className="flex gap-2">
                    ...
                    <span className="text-xs bg-gray-100 dark:bg-gray-800 px-2 py-1 rounded-full flex items-center gap-1"><Tag size={10}/>{insight.category}</span>
                  </div>
                </div>
                <p className="text-gray-600 dark:text-gray-300 mt-2 text-sm leading-relaxed">{insight.description}</p>
                <div className="flex justify-between items-center mt-4">
                  ...
                  <button 
                    ...
                    className="text-blue-500 font-medium text-sm hover:underline flex items-center gap-1 transition group-hover:gap-2"
                  >
                    Apply <Zap size={12} />
                  </button>
                </div>
                {appliedId === insight.id && (
                  <div className="mt-2 text-xs text-green-600 animate-pulse flex items-center gap-1"><CheckCircle size={12}/> Insight applied!</div>
                )}
              </div>
            );
          })}
