...
const mockOrders = [
  ...
  { 
    id: 1002, customer: 'Bob Smith', date: '2026-06-14', total: 567, 
    status: 'Processing', items: 1,
    shipping: '456 Oak Ave, Los Angeles, CA 90001',
    payment: 'Mastercard **** 5678',
    productList: ['Mechanical Keyboard x1'],
    timeline: ['Order placed', 'Processing']
  },
  { 
    id: 1003, customer: 'Carol Davis', date: '2026-06-13', total: 2340, 
    status: 'Pending', items: 4,
    shipping: '789 Pine Rd, Chicago, IL 60601',
    payment: 'Amex **** 9012',
    productList: ['4K Monitor x1', 'Laptop Stand x1', 'Wireless Mouse x1', 'USB-C Cable x2'],
    timeline: ['Order placed']
  },
  { 
    id: 1004, customer: 'David Wilson', date: '2026-06-12', total: 890, 
    status: 'Delivered', items: 2,
    shipping: '321 Elm St, Houston, TX 77001',
    payment: 'Discover **** 3456',
    productList: ['Noise Cancelling Headphones x1', 'Laptop Stand x1'],
    timeline: ['Order placed', 'Processing', 'Shipped', 'Delivered']
  },
  { 
    id: 1005, customer: 'Emma Brown', date: '2026-06-11', total: 450, 
    status: 'Processing', items: 1,
    shipping: '654 Birch Ln, Phoenix, AZ 85001',
    payment: 'Visa **** 7890',
    productList: ['Smart Watch x1'],
    timeline: ['Order placed', 'Processing']
  },
  { 
    id: 1006, customer: 'Frank Miller', date: '2026-06-10', total: 1750, 
    status: 'Pending', items: 3,
    shipping: '987 Cedar Dr, Philadelphia, PA 19101',
    payment: 'Mastercard **** 2345',
    productList: ['Gaming Laptop x1', 'Mechanical Keyboard x1', 'Wireless Mouse x1'],
    timeline: ['Order placed']
  },
  { 
    id: 1007, customer: 'Grace Lee', date: '2026-06-09', total: 320, 
    status: 'Delivered', items: 1,
    shipping: '147 Maple Ave, San Antonio, TX 78201',
    payment: 'Amex **** 6789',
    productList: ['USB-C Cable x3'],
    timeline: ['Order placed', 'Processing', 'Shipped', 'Delivered']
  },
  { 
    id: 1008, customer: 'Henry Clark', date: '2026-06-08', total: 2100, 
    status: 'Processing', items: 5,
    shipping: '258 Walnut St, San Diego, CA 92101',
    payment: 'Visa **** 0123',
    productList: ['4K Monitor x2', 'Laptop Stand x1', 'Wireless Mouse x2'],
    timeline: ['Order placed', 'Processing']
  },
];
    ...
    <div className="fixed inset-0 bg-black/60 backdrop-blur-sm flex items-center justify-center z-50 p-3 sm:p-4">
      <div className="bg-white dark:bg-gray-800 rounded-2xl p-4 sm:p-6 w-full max-w-md shadow-2xl max-h-[90vh] overflow-y-auto">
        <div className="flex flex-col sm:flex-row sm:items-center gap-3 mb-4">
          <img src={getAvatar(order.customer)} alt={order.customer} className="w-10 h-10 sm:w-12 sm:h-12 rounded-full ring-2 ring-blue-500 flex-shrink-0" />
          <div>
        ...
        <div className="space-y-3">
          <div className="flex flex-col sm:flex-row justify-between items-start sm:items-center py-2 border-b"><span className="text-gray-500">Customer</span><span className="font-medium">{order.customer}</span></div>
          <div className="flex flex-col sm:flex-row justify-between items-start sm:items-center py-2 border-b"><span className="text-gray-500">Total</span><span className="font-bold text-green-600">${order.total}</span></div>
          <div className="flex flex-col sm:flex-row justify-between items-start sm:items-center py-2 border-b"><span className="text-gray-500">Status</span><span className={`px-2 py-1 rounded-full text-xs font-medium ${order.status === 'Delivered' ? 'bg-green-100 text-green-700' : order.status === 'Processing' ? 'bg-blue-100 text-blue-700' : 'bg-yellow-100 text-yellow-700'}`}>{order.status}</span></div>
          <div className="flex flex-col sm:flex-row justify-between items-start sm:items-center py-2"><span className="text-gray-500">Items</span><span>{order.items}</span></div>
          <div className="py-2"><span className="text-gray-500">Shipping</span><p className="text-sm mt-1 break-words">{order.shipping}</p></div>
          <div className="py-2"><span className="text-gray-500">Payment</span><p className="text-sm mt-1 break-words">{order.payment}</p></div>
        </div>
  ...
  <div className="bg-white/80 dark:bg-gray-900/80 backdrop-blur-sm rounded-2xl p-3 sm:p-4 md:p-5 shadow-lg border hover:shadow-xl transition">
    <div className="flex justify-between items-start">
      <div>
        ...
        {change && <p className="text-[10px] sm:text-xs mt-1 text-green-500">+{change}% vs last month</p>}
      </div>
  ...
  return (
    <div className="space-y-4 sm:space-y-6 animate-fade-in p-3 sm:p-4 md:p-0">
      ...
      <div className="flex flex-col sm:flex-row flex-wrap gap-3 items-start sm:items-center">
        ...
        <select value={statusFilter} onChange={e => setStatusFilter(e.target.value)} className="w-full sm:w-auto border rounded-xl px-3 py-2 bg-white/70 text-sm">
        ...
        <div className="flex items-center gap-2 bg-white/70 rounded-xl px-3 py-1 border text-sm">
          ...
          <input type="number" placeholder="Min" value={minTotal} onChange={e => setMinTotal(e.target.value)} className="w-14 sm:w-16 bg-transparent outline-none text-sm" />
          <span>to</span>
          <input type="number" placeholder="Max" value={maxTotal} onChange={e => setMaxTotal(e.target.value)} className="w-14 sm:w-16 bg-transparent outline-none text-sm" />
        </div>
      </div>
      ...
      <div className="bg-white/70 dark:bg-gray-900/70 backdrop-blur-sm rounded-2xl shadow-lg overflow-hidden border border-gray-200 dark:border-gray-700">
          ...
          <div className="min-w-full inline-block align-middle">
            <table className="min-w-full divide-y divide-gray-200 dark:divide-gray-700">
              <thead className="bg-gray-100/50 dark:bg-gray-800/50">
                <tr>
                  <th className="px-2 sm:px-3 md:px-4 py-2 sm:py-3 text-left text-[10px] sm:text-xs font-semibold cursor-pointer hover:text-blue-500 whitespace-nowrap" onClick={() => handleSort('id')}>Order ID</th>
                  <th className="px-2 sm:px-3 md:px-4 py-2 sm:py-3 text-left text-[10px] sm:text-xs font-semibold cursor-pointer hover:text-blue-500 whitespace-nowrap" onClick={() => handleSort('customer')}>Customer</th>
                  <th className="hidden sm:table-cell px-2 sm:px-3 md:px-4 py-2 sm:py-3 text-left text-[10px] sm:text-xs font-semibold cursor-pointer hover:text-blue-500" onClick={() => handleSort('date')}>Date</th>
                  <th className="px-2 sm:px-3 md:px-4 py-2 sm:py-3 text-left text-[10px] sm:text-xs font-semibold cursor-pointer hover:text-blue-500 whitespace-nowrap" onClick={() => handleSort('total')}>Total</th>
                  ...
                  <th className="hidden sm:table-cell px-2 sm:px-3 md:px-4 py-2 sm:py-3 text-left text-[10px] sm:text-xs font-semibold">Items</th>
              ...
              <tbody className="divide-y divide-gray-200 dark:divide-gray-700">
                {paginated.map(order => (
                  <React.Fragment key={order.id}>
                    <tr className="hover:bg-gray-100/50 dark:hover:bg-gray-800/50 transition group">
                      <td className="px-2 sm:px-3 md:px-4 py-2 sm:py-3 md:py-4 font-mono text-xs sm:text-sm font-semibold whitespace-nowrap">#{order.id}</td>
                      <td className="px-2 sm:px-3 md:px-4 py-2 sm:py-3 md:py-4">
                        <div className="flex items-center gap-2 sm:gap-3">
                          <img src={getAvatar(order.customer)} alt={order.customer} className="w-6 h-6 sm:w-7 sm:h-7 md:w-8 md:h-8 rounded-full ring-1 ring-gray-300 dark:ring-gray-600 flex-shrink-0" />
                          <span className="font-medium text-xs sm:text-sm truncate max-w-[60px] sm:max-w-none">{order.customer}</span>
                        </div>
                      </td>
                      <td className="hidden sm:table-cell px-2 sm:px-3 md:px-4 py-2 sm:py-3 md:py-4 text-sm text-gray-500 whitespace-nowrap">{order.date}</td>
                      <td className="px-2 sm:px-3 md:px-4 py-2 sm:py-3 md:py-4 text-xs sm:text-sm font-bold text-green-600 whitespace-nowrap">${order.total}</td>
                      <td className="px-2 sm:px-3 md:px-4 py-2 sm:py-3 md:py-4 whitespace-nowrap">
                        <span className={`inline-flex items-center gap-1 px-1.5 sm:px-2 py-0.5 sm:py-1 rounded-full text-[8px] sm:text-[10px] md:text-xs font-medium ${
                      ...
                      <td className="hidden sm:table-cell px-2 sm:px-3 md:px-4 py-2 sm:py-3 md:py-4 text-sm">{order.items}</td>
                      <td className="px-2 sm:px-3 md:px-4 py-2 sm:py-3 md:py-4">
                        <button onClick={() => toggleExpand(order.id)} className="text-gray-500 hover:text-blue-500 transition">
                          {expandedRow === order.id ? <ChevronUp size={16} className="sm:size-18" /> : <ChevronDown size={16} className="sm:size-18" />}
                        </button>
                      </td>
                      <td className="px-2 sm:px-3 md:px-4 py-2 sm:py-3 md:py-4">
                        <button onClick={() => setSelectedOrder(order)} className="text-blue-500 hover:text-blue-700 transition"><Eye size={14} className="sm:size-16" /></button>
                      </td>
                    </tr>
                    {/* Expandable details row */}
                    {expandedRow === order.id && (
                      <tr className="bg-gray-50/50 dark:bg-gray-800/30">
                        <td colSpan="8" className="px-2 sm:px-3 md:px-4 py-2 sm:py-3 md:py-4 text-sm">
                          <div className="grid grid-cols-1 sm:grid-cols-2 gap-2 sm:gap-4 text-sm">
                            <div>
                              <p className="font-semibold flex items-center gap-1"><MapPin size={14} /> Shipping</p>
                              <p className="text-gray-600 dark:text-gray-400 break-words">{order.shipping}</p>
                            </div>
                            <div>
                              <p className="font-semibold flex items-center gap-1"><CreditCard size={14} /> Payment</p>
                              <p className="text-gray-600 dark:text-gray-400 break-words">{order.payment}</p>
                            </div>
                            <div className="sm:col-span-2">
                              <p className="font-semibold flex items-center gap-1"><ShoppingBag size={14} /> Products</p>
                              <ul className="list-disc list-inside text-gray-600 dark:text-gray-400">
                                {order.productList.map((item, idx) => <li key={idx} className="break-words">{item}</li>)}
                              </ul>
                            </div>
                            <div className="sm:col-span-2">
                              ...
                              <div className="flex flex-wrap gap-2 mt-1">
                                {order.timeline.map((step, idx) => (
                                  <span key={idx} className="inline-flex items-center gap-1 px-2 py-0.5 rounded-full bg-gray-200 dark:bg-gray-700 text-[10px] sm:text-xs">
                ...
                {paginated.length === 0 && (
                  <tr><td colSpan="8" className="text-center py-8 text-gray-400 text-sm">No orders match your filters</td></tr>
                )}
              </tbody>
            </table>
          </div>
        ...
        {totalPages > 1 && (
          <div className="flex justify-between items-center p-2 sm:p-3 md:p-4 border-t border-gray-200 dark:border-gray-700">
            <button disabled={currentPage===1} onClick={() => setCurrentPage(p => p-1)} className="px-2 sm:px-3 py-1 rounded bg-gray-200 dark:bg-gray-700 disabled:opacity-50 text-xs sm:text-sm"><ChevronLeft size={14} className="sm:size-16" /></button>
            <span className="text-xs sm:text-sm text-gray-600 dark:text-gray-300">Page {currentPage} of {totalPages}</span>
            <button disabled={currentPage===totalPages} onClick={() => setCurrentPage(p => p+1)} className="px-2 sm:px-3 py-1 rounded bg-gray-200 dark:bg-gray-700 disabled:opacity-50 text-xs sm:text-sm"><ChevronRight size={14} className="sm:size-16" /></button>
          </div>
        )}
      </div>
