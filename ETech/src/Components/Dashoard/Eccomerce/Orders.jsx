...
// Generate realistic avatars using randomuser.me (random gender per order)
...
// Order details modal (already present, keep as is)
// ... (I'll keep the OrderModal from earlier, but now we can also show shipping/payment)
  ...
  return (
    <div className="space-y-6 animate-fade-in">
      <div className="flex flex-wrap justify-between items-center gap-4">
        <div>
          <h1 className="text-4xl font-black bg-gradient-to-r from-gray-900 to-gray-600 dark:from-white dark:to-gray-400 bg-clip-text text-transparent">Orders</h1>
        ...
        <div className="flex gap-2">
          ...
          <button onClick={() => { setRefreshing(true); fetchOrders(true); }} disabled={refreshing} className="bg-gray-600 hover:bg-gray-700 text-white px-3 py-2 rounded-xl shadow"><RefreshCw size={18} className={refreshing ? 'animate-spin' : ''} /></button>
        </div>
      </div>
      ...
      <div className="flex flex-wrap gap-3 items-center">
        <div className="relative flex-1 max-w-md">
          ...
          <input type="text" placeholder="Search ID or customer..." value={search} onChange={e => setSearch(e.target.value)} className="w-full pl-9 pr-3 py-2 border rounded-xl bg-white/70 dark:bg-gray-900/70" />
        </div>
        <select value={statusFilter} onChange={e => setStatusFilter(e.target.value)} className="border rounded-xl px-3 py-2 bg-white/70">
          <option value="all">All Status</option><option>Pending</option><option>Processing</option><option>Delivered</option>
        </select>
        <div className="flex items-center gap-2 bg-white/70 rounded-xl px-3 py-1 border">
          <Calendar size={16} /><input type="date" value={dateRange.start} onChange={e => setDateRange({...dateRange, start: e.target.value})} className="bg-transparent outline-none text-sm" />
          <span>to</span><input type="date" value={dateRange.end} onChange={e => setDateRange({...dateRange, end: e.target.value})} className="bg-transparent outline-none text-sm" />
        </div>
        <div className="flex items-center gap-2 bg-white/70 rounded-xl px-3 py-1 border">
          <DollarSign size={16} />
        ...
        <div className="overflow-x-auto">
          <table className="min-w-full">
            <thead className="bg-gray-100/50 dark:bg-gray-800/50">
              <tr>
                <th className="px-4 py-3 text-left text-xs font-semibold cursor-pointer hover:text-blue-500" onClick={() => handleSort('id')}>Order ID</th>
                <th className="px-4 py-3 text-left text-xs font-semibold cursor-pointer hover:text-blue-500" onClick={() => handleSort('customer')}>Customer</th>
                <th className="px-4 py-3 text-left text-xs font-semibold cursor-pointer hover:text-blue-500" onClick={() => handleSort('date')}>Date</th>
                <th className="px-4 py-3 text-left text-xs font-semibold cursor-pointer hover:text-blue-500" onClick={() => handleSort('total')}>Total</th>
            ...
            <tbody className="divide-y divide-gray-200 dark:divide-gray-700">
              {paginated.map(order => (
                <React.Fragment key={order.id}>
                  <tr className="hover:bg-gray-100/50 dark:hover:bg-gray-800/50 transition group">
                    <td className="px-4 py-4 font-mono text-sm font-semibold">#{order.id}</td>
                    <td className="px-4 py-4">
                      <div className="flex items-center gap-3">
                        <img src={getAvatar(order.customer)} alt={order.customer} className="w-8 h-8 rounded-full ring-1 ring-gray-300 dark:ring-gray-600" />
                        <span className="font-medium">{order.customer}</span>
                      </div>
                    </td>
                    <td className="px-4 py-4 text-sm text-gray-500">{order.date}</td>
                    <td className="px-4 py-4 text-sm font-bold text-green-600">${order.total}</td>
                    <td className="px-4 py-4">
                    ...
                    <tr className="bg-gray-50/50 dark:bg-gray-800/30">
                      <td colSpan="8" className="px-6 py-4">
                        <div className="grid grid-cols-1 md:grid-cols-2 gap-4 text-sm">
                          <div>
                            <p className="font-semibold flex items-center gap-1"><MapPin size={14} /> Shipping Address</p>
                            <p className="text-gray-600 dark:text-gray-400">{order.shipping}</p>
                          </div>
                          <div>
                            <p className="font-semibold flex items-center gap-1"><CreditCard size={14} /> Payment Method</p>
              ...
              {paginated.length === 0 && (
                <tr><td colSpan="8" className="text-center py-12 text-gray-400">No orders match your filters</td></tr>
              )}
            </tbody>
          </table>
        </div>
