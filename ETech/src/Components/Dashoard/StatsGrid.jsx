        ...
        <div className="bg-white/80 dark:bg-slate-900/80 backdrop-blur-xl rounded-xl p-4 sm:p-6 border border-slate-200/50 dark:border-slate-700/50 hover:shadow-xl transition-all duration-300 group" key={index}>
          <div className="flex flex-col sm:flex-row items-start sm:items-center justify-between gap-2">
            <div className="flex-1 w-full">
              ...
              <p className='text-xl sm:text-2xl md:text-3xl font-bold text-slate-800 dark:text-white mb-2 sm:mb-3'>{stat.value}</p>
              <div className="flex flex-wrap items-center gap-1">
                {stat.trend === "up" ? <ArrowUpRight className='w-3 h-3 sm:w-4 sm:h-4 text-emerald-500'/> : <ArrowDownRight className='w-3 h-3 sm:w-4 sm:h-4 text-red-500'/>}
                <span className={`text-xs sm:text-sm font-semibold ${stat.trend === "up" ? "text-emerald-500" : "text-red-500"}`}>{stat.change}</span>
                <span className='text-[10px] sm:text-xs text-slate-500 dark:text-slate-400'>Vs Last Month</span>
              </div>
            </div>
          ...
          <div className="mt-2 sm:mt-3 h-1 sm:h-1.5 bg-slate-100 dark:bg-slate-800 rounded-full overflow-hidden">
