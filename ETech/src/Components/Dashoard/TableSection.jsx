          ...
          <div className="min-w-full inline-block align-middle">
            <table className="min-w-full divide-y divide-slate-200/50 dark:divide-slate-700/50">
              <thead className="bg-slate-50/50 dark:bg-slate-800/50">
                <tr>
                  <th className="text-left p-3 sm:p-4 text-xs font-semibold text-slate-600">Order</th>
                  <th className="text-left p-3 sm:p-4 text-xs font-semibold text-slate-600 hidden sm:table-cell">Customer</th>
                  <th className="text-left p-3 sm:p-4 text-xs font-semibold text-slate-600 hidden md:table-cell">Product</th>
                  <th className="text-left p-3 sm:p-4 text-xs font-semibold text-slate-600">Amount</th>
                  <th className="text-left p-3 sm:p-4 text-xs font-semibold text-slate-600 hidden lg:table-cell">Status</th>
                  <th className="text-left p-3 sm:p-4 text-xs font-semibold text-slate-600">Date</th>
                </tr>
              </thead>
              <tbody className="divide-y divide-slate-200/50 dark:divide-slate-700/50">
                {recentOrders.map((order, index) => (
                  <tr className="hover:bg-slate-50/50 dark:hover:bg-slate-800/50 transition-colors" key={index}>
                    <td className="p-3 sm:p-4"><span className="text-xs sm:text-sm font-medium text-blue-600">{order.id}</span></td>
                    <td className="hidden sm:table-cell p-3 sm:p-4"><span className="text-sm text-slate-800 dark:text-white">{order.customer}</span></td>
                    <td className="hidden md:table-cell p-3 sm:p-4"><span className="text-sm text-slate-800 dark:text-white">{order.product}</span></td>
                    <td className="p-3 sm:p-4"><span className="text-sm font-semibold text-slate-800 dark:text-white">{order.amount}</span></td>
                    <td className="hidden lg:table-cell p-3 sm:p-4"><span className={`text-xs px-2 py-1 rounded-full font-medium ${getStatusColor(order.status)}`}>{order.status}</span></td>
                    <td className="p-3 sm:p-4"><span className="text-xs sm:text-sm text-slate-500">{order.date}</span></td>
                    <td className="p-3 sm:p-4"><MoreHorizontal className="w-4 h-4 text-slate-400" /></td>
                  </tr>
                ))}
              </tbody>
            </table>
      ...
      <div className="bg-white/80 dark:bg-slate-900/80 backdrop-blur-xl rounded-2xl border border-slate-200/50 dark:border-slate-700/50 overflow-hidden">
        ...
        <div className="p-4 sm:p-6 space-y-4">
          {topProducts.map((product, index) => (
            <div className="flex flex-col sm:flex-row sm:items-center justify-between p-3 sm:p-4 rounded-xl hover:bg-slate-50 dark:hover:bg-slate-800/50 transition-colors gap-2" key={index}>
